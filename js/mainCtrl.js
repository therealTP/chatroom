...
app.controller('mainCtrl', function($scope, parseService){
  ...
  $scope.getParseData = function() {
    ...
    .then(
      function(response) {
      $scope.messages = response.results; // is this right?
      }
    );
  };
  ...
  $scope.sortByDate = function() {
    $scope.sortOldToNew = !$scope.sortOldToNew;
  };
  ...
  $scope.postData = function() {
    ...
    .then(
      function(response) {
        $scope.message = '';
        console.log('Success! Code of: ' + response.status);
        $scope.getParseData();
      },
      function(response) {
        console.log('Error. Code of: ' + response.status);
      }
    );
  };
